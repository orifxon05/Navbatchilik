    ...
    with st.form("login_form"):
        ...
        if submit_button:
            ...
            if password_clean == "admin05":
                ...
                st.rerun()
            # 4-etaj paroli
            elif password_clean == st.secrets["password"]:
                reset_login_attempts()
                st.session_state["password_correct"] = True
...
if st.session_state.get("is_admin", False):
    ...
    tab1, tab2, tab3 = st.tabs(["📤 FAYL YUKLASH", "⚙️ SOZLAMALAR", "📊 MA'LUMOTLAR"])
    
    with tab1:
        st.subheader("📤 Excel/CSV Fayl Yuklash")
        st.info("""
        **Format:** Excel yoki CSV fayl
        
        **Ustunlar (tartib bo'yicha):**
        1. Ism
        2. Familiya
        3. Xona raqami
        4. Telefon raqami
        ...
        **Diqqat:** Fayl yuklangandan so'ng Google Sheets'ga avtomatik ko'chiriladi!
        """)
        
        # Qaysi etajga yuklash
        target_floor = st.selectbox("🏢 Qaysi etajga yuklash?", 
                                     ["4-etaj (O'g'il bolalar)", "3-etaj (Qizlar)"])
        floor_key = "4-etaj" if "4-etaj" in target_floor else "3-etaj"
        sheet_name = FLOOR_CONFIG[floor_key]["sheet_name"]
        
        st.info(f"📝 Google Sheet: **{sheet_name}**")
        
        uploaded_file = st.file_uploader("Excel yoki CSV faylni tanlang", 
                                         type=['xlsx', 'xls', 'csv'])
        ...
        if uploaded_file is not None:
            try:
                ...
                st.success(f"✅ Fayl yuklandi: {len(upload_df)} ta qator")
                st.dataframe(upload_df, use_container_width=True)
                
                # Tizim headerlariga moslashtirish
                st.subheader("📋 Ustun nomlari moslashtirish")
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.write("**Sizning faylingiz:**")
                    for i, col in enumerate(upload_df.columns):
                        st.write(f"{i+1}. {col}")
                
                with col2:
                    st.write("**Google Sheets formatiga:**")
                    st.write("1. Ism")
                    st.write("2. Familiya")
                    st.write("3. Xona raqami")
                    st.write("4. Telefon raqami")
                    st.write("5. Telegram ID")
                
                # Mapping
                expected_cols = ["Ism", "Familiya", "Xona raqami", "Telefon raqami", "Telegram ID"]
                
                st.info("📍 Avtomatik moslashtirish amalga oshiriladi...")
                
                # Tasdiqlash
                if st.button("🚀 Google Sheets'ga yuklash", type="primary"):
                    try:
                        # Google Sheets client
                        client = get_client()
                        target_sheet = client.open(sheet_name).sheet1
                        
                        # Hozirgi ma'lumotlarni o'qish
                        existing_data = target_sheet.get_all_values()
                        
                        # Agar jadval bo'sh bo'lsa, headerlar qo'shish
                        if not existing_data:
                            target_sheet.append_row(expected_cols)
                        
                        # Ma'lumotlarni qo'shish
                        rows_added = 0
                        for _, row in upload_df.iterrows():
                            row_data = []
                            for col in upload_df.columns[:5]:  # Faqat birinchi 5 ta ustun
                                row_data.append(str(row[col]) if pd.notna(row[col]) else "")
                            
                            # Kamayuv ustunlarni to'ldirish
                        ...
                        st.success(f"✅ {rows_added} ta talaba Google Sheets'ga yuklandi!")
                        send_telegram_alert(f"📤 ADMIN: {rows_added} ta talaba yuklandi ({sheet_name})")
                        
                    except Exception as e:
                        st.error(f"❌ Xatolik: {e}")
                        send_telegram_alert(f"❌ ADMIN: Yuklashda xatolik - {str(e)[:100]}")
            
            except Exception as e:
                st.error(f"❌ Faylni o'qishda xatolik: {e}")
    
    with tab2:
        st.subheader("⚙️ Tizim Sozlamalari")
        
        st.warning("⚠️ **Diqqat:** Sozlamalar `.streamlit/secrets.toml` faylida saqlanadi.")
        
        # Hozirgi sozlamalar
        st.markdown("### 📋 Hozirgi Sozlamalar")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.metric("4-etaj Google Sheet", FLOOR_CONFIG["4-etaj"]["sheet_name"])
            st.metric("4-etaj Telegram Guruh", FLOOR_CONFIG["4-etaj"]["telegram_group"])
            st.code(f"4-etaj Paroli: {st.secrets.get('password', 'NOT SET')}", language="text")
        
        with col2:
            st.metric("3-etaj Google Sheet", FLOOR_CONFIG["3-etaj"]["sheet_name"])
            st.metric("3-etaj Telegram Guruh", FLOOR_CONFIG["3-etaj"]["telegram_group"])
            st.code(f"3-etaj Paroli: {st.secrets.get('password_3etaj', 'NOT SET')}", language="text")
        
        st.markdown("---")
        st.metric("Telegram Bot Token", TELEGRAM_TOKEN[:20] + "...")
        st.metric("Admin Telegram ID", ADMIN_CHAT_ID)
        
        st.markdown("---")
        st.info("""
        **Sozlamalarni tahrirlash uchun:**
        
        1. `.streamlit/secrets.toml` faylini oching
        2. Kerakli qiymatlarni o'zgartiring
        3. Saqlang va ilovani qayta ishga tushiring
        
        **Masalan:**
        ```toml
        password = "yangi_parol_4etaj"
        password_3etaj = "yangi_parol_3etaj"
        ```
        """)
    
    with tab3:
        st.subheader("📊 Google Sheets Ma'lumotlari")
        
        floor_select = st.selectbox("Qaysi etajning ma'lumotlarini ko'rish?",
                                     ["4-etaj (O'g'il bolalar)", "3-etaj (Qizlar)"],
                                     key="admin_floor_select")
        
        floor_key = "4-etaj" if "4-etaj" in floor_select else "3-etaj"
        sheet_name_view = FLOOR_CONFIG[floor_key]["sheet_name"]
        
        try:
            client = get_client()
            view_sheet = client.open(sheet_name_view).sheet1
            view_data = view_sheet.get_all_values()
            
            if view_data:
                view_df = pd.DataFrame(view_data[1:], columns=view_data[0])
                st.dataframe(view_df, use_container_width=True)
                st.info(f"📊 Jami: {len(view_df)} ta talaba")
            else:
                st.warning("Ma'lumot topilmadi")
        except Exception as e:
            st.error(f"Xatolik: {e}")
    
    st.stop()  # Admin panel'dan keyin oddiy interfeys ko'rsatilmasin
...
# ============================================================================
# 🔧 ADMIN PANEL (faqat admin05 kirganda ko'rinadi)
# ============================================================================
if st.session_state.get("is_admin", False):
    st.markdown("## 🔧 ADMIN PANEL")
    st.info("Super Admin rejimi: Barcha tashkilotlarni boshqarish")
    
    # Admin tabs
    admin_tab1, admin_tab2, admin_tab3 = st.tabs(["🏢 Tashkilotlar", "📊 Statistika", "⚙️ Sozlamalar"])
    
    with admin_tab1:
        st.subheader("🏢 Ro'yxatdagi Tashkilotlar")
        ...
        for floor_id, config in FLOOR_CONFIG.items():
            orgs_data.append({
                ...
                "Telegram Guruh": config["telegram_group"],
                "Holat": "✅ Aktiv"
            })
        ...
        st.subheader("➕ Yangi Tashkilot Qo'shish")
        
        with st.form("add_org_form"):
            col1, col2 = st.columns(2)
            
            with col1:
                new_org_id = st.text_input("ID (masalan: tatu-1etaj)", placeholder="faqat kichik harf va tire")
                new_org_name = st.text_input("Tashkilot nomi", placeholder="TATU 1-etaj Yotoqxona")
                new_org_password = st.text_input("Parol", placeholder="tatu1234")
            
            with col2:
                new_org_sheet = st.text_input("Google Sheet nomi", placeholder="TATU_Navbatchilik")
                new_org_telegram = st.text_input("Telegram guruh ID", placeholder="-1002345...")
                new_org_admin_id = st.text_input("Admin Telegram ID", placeholder="123456789")
            
            submit_org = st.form_submit_button("✅ Qo'shish", type="primary", use_container_width=True)
            
            if submit_org:
                if new_org_id and new_org_name and new_org_password:
                    st.success(f"✅ Yangi tashkilot '{new_org_name}' qo'shildi!")
                    st.info("⚠️ ESLATMA: Hozircha bu demo versiya. Kod ichida FLOOR_CONFIG ni yangilash kerak.")
                else:
                    st.warning("⚠️ Barcha maydonlarni to'ldiring!")
    ...
    with admin_tab2:
        ...
        with col1:
            st.metric("🏢 Tashkilotlar", len(FLOOR_CONFIG))
        
        with col2:
            st.metric("📱 Telefon Agentlar", "2 ta")
        
        with col3:
            st.metric("📨 SMS (Bugun)", "0")
        
        with col4:
            st.metric("💰 Xarajat (Bugun)", "0 so'm")
        
        st.markdown("---")
        st.info("📊 To'liq statistika keyinroq qo'shiladi...")
    
    with admin_tab3:
        st.subheader("⚙️ Tizim Sozlamalari")
        
        st.markdown("##### 🔐 Xavfsizlik")
        st.text_input("Admin parol", value="admin05", type="password", disabled=True)
        st.caption("Parolni o'zgartirish uchun kodda ADMIN_PASSWORD ni yangilang")
        
        st.markdown("---")
        
        st.markdown("##### 🤖 Telegram Bot")
        st.text_input("Bot Token", value=TELEGRAM_TOKEN[:20] + "...", disabled=True)
        st.text_input("Admin Chat ID", value=ADMIN_CHAT_ID, disabled=True)
        
        st.markdown("---")
        
        st.markdown("##### 📂 Ma'lumotlar")
        st.info("Barcha ma'lumotlar Google Sheets'da saqlanadi")
        
        if st.button("🔄 Tizimni Qayta Yuklash"):
            st.cache_resource.clear()
            st.success("✅ Kesh tozalandi!")
    
    st.stop()  # Admin paneldan chiqqanda boshqa interfeys ko'rinmasin

# ============================================================================
# ODDIY FOYDALANUVCHI INTERFEYSI (admin bo'lmasa)
# ============================================================================
# Streamlit radio orqali ham navigatsiya (fallback)
